...
exports.parse = function parse(source, opts){
    ...
    function setDontEnumProperty(object, key, value){
    ...
    var instrumentNodes = function(node, parent, prev, next){
        setDontEnumProperty(node, "parent", parent);
        setDontEnumProperty(node, "prev", prev);
        setDontEnumProperty(node, "next", next);
        setDontEnumProperty(node, "depth", parent ? parent.depth + 1 : 0); // used later for moonwalk

        setDontEnumProperty(node, "toString", toString);
        ...
        setDontEnumProperty(node, "startToken", prevToken? getNodeStartToken(prevToken, node.range) : ast.tokens[0]);
        setDontEnumProperty(node, "endToken", nextToken? getNodeEndToken(nextToken, node.range) : ast.tokens[ast.tokens.length - 1]);
    };
